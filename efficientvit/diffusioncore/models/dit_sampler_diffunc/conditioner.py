...
class Conditioner:
    ...
    def p_transform(self, x: Tensor, t: int, clip_denoised, denoised_fn) -> Tensor:
        """
        Args:
            x (Tensor, dtype=float, shape=(B, 3, H, W)): normalized image
                at timestep t.
        ...
        if t == -1:
            ...
            preds = self.diffusion.model_predictions(x, t, x_self_cond=False, clip_x_start=False, clip_denoised=clip_denoised, denoised_fn=denoised_fn)  # (B, 3, H, W)
            return preds.pred_x_start  # (B, 3, H, W)

    @torch.enable_grad()  # important since we use torch.autograd.grad!
    def __call__(self, x: Tensor, t: int, x_targ: Tensor, clip_denoised, denoised_fn) -> Tensor:
        """
        Args:
            x (Tensor, dtype=float, shape=(B, 3, H, W)): normalized image
            t (int): the timestep in [-1, T-2]
        ...
        if self.apply_p_transform:
            x_in = self.p_transform(x_in, t, clip_denoised, denoised_fn)  # (B, 3, H, W)
